...
export default function CatalystPanel({ events, marketAnalysis, setEvents }: Props) {
  ...
  return <Panel label="市场监测 · 本月日历与数据" count={visible.length} actions={
    ...
    {marketAnalysis?.analyses[activeCategory]&&<div className={`mb-2 border-l-2 p-2 ${ANALYSIS_STYLE[activeCategory]}`}><div className="flex items-center gap-2 font-mono2 text-[9px] mb-1"><span>AI 综合判断</span><span className="t4">{marketAnalysis.provider||'OpenAI'} · {marketAnalysis.model} · {marketAnalysis.prompt_version||'v1'} · {marketAnalysis.analyses[activeCategory].fact_count}项事实</span></div><p className="text-[11px] t2 leading-relaxed whitespace-pre-wrap">{marketAnalysis.analyses[activeCategory].text}</p></div>}
